function validateForm(data, errors, setErrors) {
    ...
    if (!data.image.includes("https://"))
        newErrors.image = "Must be a URL or link";
    else {
        if (!data.image.includes(".png"))
            newErrors.image = "The image must be a .png";
    ...
    // * verification
    if(data.name&&data.image&&data.up&&data.attack&&data.defense&&data.types.length) {
        newErrors.form = ""
        setErrors(newErrors);
        return true
